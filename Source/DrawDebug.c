...
void DrawDebug(HDC hdc, HDC hMemDC, INT mx, INT my, UINT width, UINT height)
{
	...
	TextOut(hMemDC, ctrl_pane.lWidth + 10, 10, str, lstrlen(str));
	wsprintf(str, L"mouseX: %d, mouseY: %d", mx, my);
	TextOut(hMemDC, ctrl_pane.lWidth + 10, 30, str, lstrlen(str));
	wsprintf(str, L"ctrl_width: %d", ctrl_pane.lWidth);
	TextOut(hMemDC, ctrl_pane.lWidth + 10, 50, str, lstrlen(str));
	//wsprintf(str, L"%s", ctrl_pane.mHover ? L"TRUE" : L"FALSE");
	wsprintf(str, L"HOVER[0]: %s", sliders[0].mHover ? L"TRUE" : L"FALSE");
	TextOut(hMemDC, ctrl_pane.lWidth + 10, 70, str, lstrlen(str));
	//wsprintf(str, L"%s", ctrl_pane.mDrag ? L"TRUE" : L"FALSE");
	wsprintf(str, L"DRAG[0]: %s", sliders[0].mDrag ? L"TRUE" : L"FALSE");
	TextOut(hMemDC, ctrl_pane.lWidth + 10, 90, str, lstrlen(str));
	swprintf(str, sizeof(str) / sizeof(str[0]), L"VALUE[0]: %.3Lf", sliders[0].value);
	TextOut(hMemDC, ctrl_pane.lWidth + 10, 110, str, lstrlen(str));
	swprintf(str, sizeof(str) / sizeof(str[0]), L"VALUE[1]: %.3Lf", sliders[1].value);
	TextOut(hMemDC, ctrl_pane.lWidth + 10, 130, str, lstrlen(str));
	swprintf(str, sizeof(str) / sizeof(str[0]), L"VALUE[2]: %.3Lf", sliders[2].value);
	TextOut(hMemDC, ctrl_pane.lWidth + 10, 150, str, lstrlen(str));
	swprintf(str, sizeof(str) / sizeof(str[0]), L"VALUE[3]: %.3Lf", sliders[3].value);
	TextOut(hMemDC, ctrl_pane.lWidth + 10, 170, str, lstrlen(str));
